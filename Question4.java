class BankAccount {
    ...
    public void changeBalance(double nb){
        this.balance = nb;
    }
    //i added these last 2 functions to help with the withdraw function
}

class CheckingAccount extends BankAccount {
    private double overdraftlimit;

    CheckingAccount(int aN, String aH, double b, String at, double odl) {
        super(aN, aH, b, at);
        this.overdraftlimit = odl;
        

    }
    //CheckingAccount inherits from BankAccount with the new double variable 
    //overdraftlimit, and the constructor uses super() to assign the BankAccount
    //variables, and also assigns overdraftlimit
    public String withdraw (double amount) {
        odlb = super.getBalance() + this.overdraftlimit;
        if (odlb< amount) {return "Insufficient Balance";}
        else {super.changeBalance(super.getBalance() - amount);; return null;}
        //takes double amount and checks if it is more than the present balance + overdraftlimit. If so, 
        //returns string "Insufficient Balance". The assignment states to return it, not print it
        //so I went with returning it. If amount is less than current balance, it removes that 
        //amount from balance using the changeBalance and getBalance from the super class. 
    }

    public double displayOverdraftlimit(){
        return this.overdraftlimit;
    }//returns overdraftlimit
}
    class BankAccountTest {
        ...
        public static void main (String[] args) {
            ...
            CheckingAccount checkacc = new CheckingAccount(12345, "John Doe", 1000, "CHECKING", 0);
       ...
       //creates CheckingAccount with those parameters.
    }
}